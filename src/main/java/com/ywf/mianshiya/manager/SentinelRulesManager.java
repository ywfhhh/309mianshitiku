package com.ywf.mianshiya.manager;
...
public class SentinelRulesManager {
    ...
    public void listenRules() throws Exception {
        ...
        String authRulePath = new File(sentinelDir, "AuthRule.json").getAbsolutePath();
        ...
        FileRefreshableDataSource<List<AuthorityRule>> authRuleDataSource
                = new FileRefreshableDataSource<>(
                authRulePath, authRuleListParser);
        DegradeRuleManager.register2Property(degradeRuleDataSource.getProperty());
        WritableDataSource<List<AuthorityRule>> authWds = new FileWritableDataSource<>(authRulePath, this::encodeJson);
        // Register to writable data source registry so that rules can be updated to file
        WritableDataSourceRegistry.registerAuthorityDataSource(authWds);
    }
    ...
    private Converter<String, List<AuthorityRule>> authRuleListParser = source -> JSON.parseObject(source,
